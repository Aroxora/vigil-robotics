...
const ENFORCE_READ_BEFORE_EDIT = process.env['VIGIL_ENFORCE_READ_BEFORE_EDIT'] !== 'false';
...
export function validateReadForEdit(
  ...
  // No read record at all — skip guard in test environments
  if (!record) {
    if (process.env.VITEST || process.env.NODE_ENV === 'test' || process.env.TEST) return null;
    return [
