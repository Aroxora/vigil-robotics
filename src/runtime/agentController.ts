...
export class AgentController implements IAgentController {
  ...
  async *send(message: string): AsyncIterableIterator<AgentEventUnion> {
    ...
    while (fallbackAttempts < AgentController.MAX_FALLBACK_ATTEMPTS) {
      // Sanitize history before each send to remove orphaned tool_calls
      // that would cause "insufficient tool messages" API errors.
      ...
      const timeoutMsRaw = process.env['VIGIL_AGENT_RUN_TIMEOUT_MS'];
      ...
      // configured run-timeout (e.g. VIGIL_AGENT_RUN_TIMEOUT_MS=120000)
  ...
  private findFallbackProvider(): { provider: ProviderId; model: string } | null {
    ...
    const preferenceOrder: ProviderId[] = ['deepseek'];
