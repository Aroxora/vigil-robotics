...
const AUTO_UPDATE_STATE_FILE = join(homedir(), '.vigil', 'update-state.json');
...
export async function checkForUpdates(currentVersion: string): Promise<UpdateInfo | null> {
  ...
  try {
    // CRITICAL: must use the SCOPED package name `anvilwing`.
    // The unscoped name `vigil-coder` on npm is a DIFFERENT package
    // by an unrelated author (last published v1.0.86) — not us, not our
    // brand. Querying it returns a stale third-party version that looks
    // "older" than ours, so the update check silently never fires and
    // users sit on outdated builds until they manually reinstall. Real
    // bug we shipped before the package was scoped; do not let a future
    // refactor "simplify" this back to the unscoped name.
    const { stdout } = await execAsync('npm view @anvilwing/vigil version', {
      timeout: 5000,
      encoding: 'utf8',
    });
...
export async function maybeAutoUpdate(
  ...
  options.logger?.(
    theme.info(`Updating vigil to v${updateInfo.latest} (current v${updateInfo.current})...`)
  );
...
export function formatUpdateNotification(updateInfo: UpdateInfo, note?: string): string {
  ...
  lines.push(
    padLine('Run: npm install -g @anvilwing/vigil@latest', theme.primary),
    theme.warning(`╰${border}╯`),
    ''
  );
...
function resolveAutoUpdateEnabled(): { enabled: boolean; reason?: string } {
  const disabledEnv = process.env['VIGIL_DISABLE_AUTO_UPDATE'];
  const autoEnv = process.env['VIGIL_AUTO_UPDATE'];
...
function resolveUpdateCommand(): string {
  const override = process.env['VIGIL_UPDATE_COMMAND'];
  if (override && override.trim()) {
    return override.trim();
  }
  return 'npm install -g @anvilwing/vigil@latest';
}
...
const SESSION_STATE_FILE = join(homedir(), '.vigil', 'session-state.json');
  ...
  try {
    ...
    const child = spawn(process.execPath, [cliPath, ...args], {
      ...
      env: {
        ...process.env,
        VIGIL_RESUMED: 'true',
        VIGIL_RESUME_VERSION: updateInfo.latest,
      },
    });
