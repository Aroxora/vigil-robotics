...
const IGNORED_DIRS = new Set([
  '.git', 'node_modules', 'dist', '.vigil', 'build', 'coverage', '.next', 'out',
...
export function resolveWorkspaceCaptureOptions(env: NodeJS.ProcessEnv = process.env): WorkspaceCaptureOptions {
  return {
    treeDepth: parsePositiveInt(env['VIGIL_CONTEXT_TREE_DEPTH']),
    maxEntries: parsePositiveInt(env['VIGIL_CONTEXT_MAX_ENTRIES']),
    docExcerptLimit: parsePositiveInt(env['VIGIL_CONTEXT_DOC_LIMIT']),
  };
}
